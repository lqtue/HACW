...
export async function GET({ url, platform, request }) {
  ...
  // Plain check-in counts are public (they drive the spotlight). The behaviour
  // tallies are organizer-only, so refuse a cross-site or scripted read.
  if (wantEvents && !isSameOrigin(request.headers.get('sec-fetch-site'))) {
